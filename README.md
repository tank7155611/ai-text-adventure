...
一个基于 React、TypeScript、Vite 和 OpenAI 兼容接口的本地 AI 文字冒险 Demo。

玩家从六个不同世界中选择一个开始冒险，在 100 轮结构中通过三选项推进剧情。项目不是单纯把游戏结果交给大模型决定，而是采用“前端规则层控制数值 + 大模型负责叙事表达”的方式，让剧情生成更有表现力，同时让生命、理智、攻击、防御等状态变化更稳定。
